...
while true; do
    ...
    for target_id in "${!FIRED_TARGETS[@]}"; do
        ...
        if [[ -z "${SEEN_NOW[$target_id]}" ]]; then
            ...
            MSG_HIT="ПОДТВЕРЖДЕНО: БР ID:$target_id УНИЧТОЖЕНА!"
            echo "[$SPRO_NAME] $TIMESTAMP $MSG_HIT"
            echo "$(date +%d.%m) $TIMESTAMP $SPRO_NAME $MSG_HIT" >> "$MESSAGES_LOG"
        else
            # Цель все еще летит -> ПРОМАХ
            MSG_MISS="ПРОМАХ! БР ID:$target_id продолжает движение. Готовность ко 2-му пуску."
    ...
    for ID in "${!SEEN_NOW[@]}"; do
        ...
        if [ "$V" -ge 8000 ]; then
            ...
            if [ "$CURRENT_AMMO" -le 0 ]; then
                ...
                REPORTED_IDS[$ID]=1 # Чтобы не спамить лог
